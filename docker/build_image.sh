...
Sorgente:
-v <VERSIONE>  : Imposta la versione della release da utilizzare per il build (default: ${LATEST_GOVPAY_CONSOLE_RELEASE})
-l <DIR>       : Usa una directory locale contenente i file della build Angular (contenuto di dist/govpay-console/browser)
...
LATEST_LINK="$(curl -qw '%{redirect_url}\n' https://github.com/link-it/govpay-console/releases/latest 2> /dev/null)"
...
# Build imagine govpay-console

if [ -z "$TAG" ]
then
    REPO=${REGISTRY_PREFIX}/govpay-console
