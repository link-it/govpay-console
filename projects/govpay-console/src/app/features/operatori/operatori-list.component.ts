...
export class OperatoriListComponent implements OnInit {
  ...
  readonly viewMode = computed<'table' | 'rows'>(() => this.viewModeOverride() ?? this.prefView.featureView('operatori') ?? this.viewModeDefault());
  ...
  constructor() {
    ...
    inject(DestroyRef).onDestroy(
      tweaks.register({
        ...
        onReset: () => {
          (this.prefView.setFeatureView('operatori', null), this.viewModeOverride.set(null));
  ...
  onViewModeChange(value: string): void {
    ...
    this.prefView.setFeatureView('operatori', v);
  }
  ...
  private reset(): void {
    this.listState.set(OperatoriListComponent.STATE_KEY, { search: this.searchState(), sort: this.sort() }, this.rowConfig()?.persistState ?? false);
    this.page.set(1);
    this.rows.set([]);
    this.fetch(false);
  }
