...
export class OperatoriListComponent implements OnInit {
  ...
  readonly searchFields = computed<SearchField[]>(() => {
    ...
    return [
      { id: F.principal, label: t('Operatori.Filters.Principal'), kind: 'text', placeholder: t('Operatori.Filters.PrincipalPlaceholder'), span: 2 },
      { id: F.nome, label: t('Operatori.Filters.Nome'), kind: 'text', placeholder: t('Operatori.Filters.NomePlaceholder'), span: 2 },
      {
        id: F.abilitato,
