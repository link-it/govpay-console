...
import { LnkTooltipDirective, type LnkTooltipPosition } from '../tooltip/tooltip.directive';
...
@Component({
  ...
  standalone: true,
  imports: [TranslatePipe, NgIcon, LnkTooltipDirective],
  ...
  styles: `
    ...
    /* Posizione iniziale sopra al FAB (sotto al goto-top) — vedi media
       query sotto. Su schermi piccoli max-height viene ridotta in
       modo che bottom + altezza non sfori il viewport dal top: il
       body interno (.lnk-tweaks-body) ha gia overflow-y: auto e
       scrolla quando il contenuto eccede. */
    .lnk-tweaks-panel--floating {
      bottom: 12.5rem;
      right: 1rem;
      /* 100vh - bottom (12.5rem) - top_margin (1rem) */
    ...
    /* In modalita dragged la posizione e calcolata dal pointer (con
       clamp gia fatto in onDragPointerMove); il max-height torna al
       generale 100vh - 1rem del wrapper. */
  ...
  template: `
    @if (open()) {
      ...
      <button
        ...
        [attr.aria-label]="titleKey() | translate"
        [lnkTooltip]="(tooltipKey() ?? titleKey()) | translate"
        [lnkTooltipPosition]="tooltipPosition()"
        (click)="toggle()"
      >
...
export class TweaksPanelComponent {
  ...
  readonly tooltipPosition = input<LnkTooltipPosition>('left');
