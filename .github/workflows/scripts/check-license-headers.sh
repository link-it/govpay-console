#!/bin/bash
#
# Verifica che tutti i file sorgente contengano l'header di licenza GPLv3
...
# Trova tutti i file .ts, .js, .mjs nelle directory sorgenti
...
done < <(find projects/govpay-console/src scripts -type f \( -name "*.ts" -o -name "*.js" -o -name "*.mjs" \) -print0 2>/dev/null)
