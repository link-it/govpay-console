...
SRC_DIR="$PROJECT_ROOT/projects/govpay-console/src"
...
add_header_to_file() {
  ...
  if [[ "$file" == *"/environments/version.ts"* ]]; then
    return
  fi
...
# Processa file in projects/govpay-console/src/
while IFS= read -r -d '' file; do
  add_header_to_file "$file"
done < <(find "$SRC_DIR" -type f \( -name "*.ts" -o -name "*.js" -o -name "*.mjs" -o -name "*.html" -o -name "*.css" \) -print0 2>/dev/null)

# Processa file in scripts/
while IFS= read -r -d '' file; do
  add_header_to_file "$file"
done < <(find "$SCRIPTS_DIR" -type f \( -name "*.ts" -o -name "*.js" -o -name "*.mjs" \) -print0 2>/dev/null)
